...
const Under100K = ({ addCartItem, under100k }: props) => {
                ...
                <div className="bg-white p-3 rounded-lg shadow-lg">
                  ...
                  <div className="flex flex-col  mb-4">
                    <p className="text-sm font-bold text-gray-800">
                      ₦{currencyFormater(category.price.toString())}
                    </p>
                    <p className="text-xs   line-through text-gray-800 ml-2">
                      ₦{category.original_price && category.original_price}
                    </p>
                  </div>
                  <button
                    onClick={() => addCartItem(category)}
                    className="bg-[#ff8c00] max-md:w-fit max-md:text-xs border border-transparent hover:bg-transparent hover:border-[#ff8c00] text-white hover:text-[#ff8c00] font-semibold py-2  px-4 rounded-full w-full"
                  >
                    Add to Cart
                  </button>
                </div>
